...
public class onMessage {
    public static void done(GameMessageS2CPacket packet, CallbackInfo ci){
        MinecraftClient client = MinecraftClient.getInstance();
        ClientPlayerInteractionManager manager = client.interactionManager;
        if (packet.getMessage().getString().contains("sss")){
            ci.cancel();
                    ...
                    if (target instanceof CropBlock){
                        if (state.get(((CropBlock) target).getAgeProperty()) == ((CropBlock) target).getMaxAge()) {
                            ...
                            BlockHitResult hitResult = new BlockHitResult(
                                    targetPos,
                                    Direction.UP,
                                    new BlockPos(client.player.getPos().add(0,0,-1)),
                                    false
                            );
